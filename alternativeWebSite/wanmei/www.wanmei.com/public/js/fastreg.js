(function() {	
	...
	function loadScript(url, dom, callback){
	...
	*/
		...
		*/
		...
		account: function() {
			...
			if (remote === -8) {
				...
				invokeAjaxJson('http://passport.wanmei.com/member/common/v1.do?jsonpCallback=?', {username:value,method:'checkUser'}, function(data) {
					self.account_remote = data.code
					if (data.code === 0) {
						self.showCorrect('account')
					} else {
		...
		captcha: function() {
			...
			if (remote === -3) {
				...
				*/
				invokeAjaxJson('http://passport.wanmei.com/ajax/checkrand?jsonpCallback=?', {rand: value,method:'checkRandom'}, function(data) {
					self.captcha_remote = data.code
					if (data.code === 0) {
						self.showCorrect('captcha')
					} else {
			...
			setTimeout(function() {
				...
				if (account.state === true) {
					...
					if(type == 1){	
						...
						if(mobile.state === true){	
							this.showSmsCaptcha()
						}else{
							this.$el.find('[name=mobile]').focus()	
						}
					}else {	
						var mobile = this.mobile()
						if(mobile.state === true){	
		...
		submitSmsCaptcha: function(event){
			...
			var account = this.type == 'phone' ? this.account() : this.mobile()
			var self = this
			/*
			util.jsonp('sendCode', {
				account: account.val,
			...
			*/
			invokeAjaxJson('http://passport.wanmei.com/ajax/sendcode?jsonpCallback=?', {account: account.val,type: 1,rand: value,method:'sendCode'}, function(data) {
				if(data.code === 0){
					self.hideSmsCaptcha()
					self.setCodeBtn(1)
				}else{
		...
		checkAll: function() {
			...
			switch (this.type) {
				case 'normal':
					fields = ['account', 'password', 'mobile', 'phonecode']
					break
				case 'phone':
				...
				case 'email':
					...
					fields = ['account', 'mobile', 'phonecode', 'password']
					break
			}
		...
		regSubmit: function(event) {
			...
			if (json) {
				...
				*/
				...
				invokeAjaxJson('http://passport.wanmei.com/member/fastreg/v3.do?jsonpCallback=?', params, function(data) {
					this.submit_loading = false
					if (data.code === 0) {
						app.data.username = json.account;
						if (params.type == 2) {
		...
		template: function(type) {
			...
			switch (type) {
				case 'normal':
					...
					*/
					...
					html += '<div class="qreg-input-wrap">'
					html += '<input type="text" placeholder="手机号" maxlength="11" class="qreg-input" name="mobile" tip="请输入手机号">'
					html += '</div>'
					html += '<div class="qreg-ico"></div>'
					html += '</div>'
				...
				case 'email':
					...
					html += '<div class="qreg-field qreg-field-phone">'
					html += '<div class="qreg-input-wrap">'
					html += '<input type="text" placeholder="手机号" maxlength="11" class="qreg-input" name="mobile" tip="注册后将免费享有短信服务">'
					html += '</div>'
					html += '<div class="qreg-ico"></div>'
					html += '</div>'
			...
			html += '已阅读并同意<a href="http://passport.wanmei.com/jsp/member/agreement.htm" target="_blank">用户协议及防沉迷协议</a>、<a href="http://static.wanmei.com/passport/agreement/005.html" target="_blank">隐私政策</a></label>'
			html += '</div>'
			html += '</div>'
	...
	var regSucEmail = {
		...
		template: function() {
			...
			html += '<a href="https://mail.'+ app.data.username.split('@')[1] +'" target="_blank" class="qreg-btn-tobind">打开邮箱</a>';
			html += '</div>'
			return html
		}
	}
	...
	window.fastreg = function(opt){
		...
		loadScript(httpType, scriptNow, function(){
			app.init($.extend({	
				box: '', 
				wmrid: '',
				card: '',
